...
public class BatAttackHead extends HeadAction {
    ...
    public void onRun(Player player) {
        Bukkit.broadcastMessage(player.getName() + " active <" + ChatColor.RED + "Chauvres-souris-ninjas !!" + ChatColor.WHITE + ">");
        final List<Entity> entities = new ArrayList<>();
        final List<Item> itemEntities = new ArrayList<>();
        for (Player alive : Game.getGame().getGameManager().getAlivesPlayers()) {
            if (alive != player) {
